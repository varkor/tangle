...
all:
	set -e
	curl -L -O "https://github.com/KaTeX/KaTeX/releases/download/v0.15.3/katex.zip"
	unzip katex.zip
	rm katex.zip
	mv katex src/KaTeX
...
# Update the tangle GitHub Pages application.
gh-pages:
	...
	# it's better than trying to push a broken version of tangle.
	set -e
	# It's too error-prone to clone KaTeX from the origin each time we want to push tangle, so we
	...
	git worktree add ../tangle-worktree gh-pages
	cd ../tangle-worktree
	...
	git worktree remove ../tangle-worktree -f
